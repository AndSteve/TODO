...
print($query);
$query ="SELECT userName FROM GroupUsers WHERE `userName`='$userName' AND `password`='$passWord'"; //check username / pw against database
...
if (mysqli_num_rows($result)==1) // if the query to sql matches username and password
{
    ...
    $_SESSION['user']=$user_info; // store into session superglobal
